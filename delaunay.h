...
Delaunay<T> triangulate(const std::vector<Point<T>> &points)
{
  ...
  for (auto const& pt : points) {
    ...
    qDebug() << "waiting triangulation size " << d.triangles.size();
    ...
    qDebug() << "waiting edges size: " << edges.size();
    qDebug() << "current triangulation size " << tmps.size();
    ...
    edges.erase(
        std::remove_if(edges.begin(), edges.end(),
                       [&](Edge<T>& e) { return remove[&e - &edges[0]]; }),
        edges.end());
    qDebug() << "good egdes size: " << edges.size();
    ...
    d.triangles = tmps;
    qDebug() << "latest triangulation size " << tmps.size()<< endl;
  }
  ...
  qDebug() << "final triangulation size " << d.triangles.size();
  /* Add edges. */
  for (auto const& tri : d.triangles) {
